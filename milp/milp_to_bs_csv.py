...
pipeline untouched:
    ...
    step 2 (nothing new to write -- your pipeline already supports it):
        set  terrestrial.load_bs_csv: /path/to/milp_bs.csv   in the YAML
        (or scenario.py override) and run the normal simulation sbatch.
        Placement is skipped; mobility, shadowing, capacity-aware attachment,
        PF scheduling, NTN beam allocation all run for real.
...
Faithful expansion of the MILP's own assumptions:
  ...
  - UMI_MMW sites are exported with scenario "UMI" (the enum the simulator
    ...
    (400 MHz / 28 GHz / n257 power+gain) -- the NEW-format contract that
    _load_bs_from_csv demands whenever UMI_MMW is in the config;
  - all RF columns read from the SAME YAML the MILP used, so the simulator
    judges the placement under identical physics parameters.

Usage:
    python milp_to_bs_csv.py --placement province_placement.csv \
        --config /Utilisateurs/dbenguer/ntn_tn_optim/configs/5g_base.yaml \
        --out milp_bs.csv
"""
...
def main():
    ...
    scen_cfg = (cfg.get("terrestrial", {}) or {}).get("scenarios") \
        or cfg.get("scenarios")
    if not scen_cfg:
        raise SystemExit("no scenarios block found in the YAML")
    ...
    n_mmw = sum(1 for _, r in df.iterrows() if str(r[tier_col]) == "UMI_MMW")
    ...
    if skipped:
        print(f"WARNING skipped tiers not in YAML: {skipped}")
