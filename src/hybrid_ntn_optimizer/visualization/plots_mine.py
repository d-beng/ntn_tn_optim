...
def plot_master_hybrid_animation_PLOTLn(region, users, base_stations, beam_data, user_data, duration_s, time_step_s, filename="Final_Animation.html"):
    print(f"\n🎨 Rendering Unified Visualization (Compiling frames...)")
...
def plot_master_hybrid_animation_BIG_DATA(region, users, base_stations, beam_data, user_data, duration_s, time_step_s, filename="Final_Animation.html"):
    print("🎨 Generating Big Data visualization... (This might take a moment)")

    # ==========================================
    # 1. YOUR ORIGINAL GEOMETRY GENERATORS
    # ==========================================
    hex_geojson = build_h3_geojson(region.cells)
    bs_coverage_geojson = build_bs_coverage_geojson(base_stations)
    
    # We keep time_steps for your internal logic, though Holoviews will 
    # extract the slider steps directly from the DataFrame's 'Hour' column!
    time_steps = list(range(0, duration_s + time_step_s, time_step_s))
    ...
    # ==========================================
    # 3. DATASHADER PIPELINE (The 23 Million Users)
    # ==========================================
    ds_points = hv.Dataset(df, kdims=['Lon', 'Lat', 'Hour', 'State'])
    points = ds_points.to(hv.Points, kdims=['Lon', 'Lat'], vdims=['State'], groupby='Hour')

    shaded_users = datashade(
        points, 
        aggregator=ds.count_cat('State'), 
        color_key=color_key
    ).opts(
        width=1000, 
        height=800, 
        bgcolor='black', 
        title="Master Hybrid Network Simulation"
    )
    
    shaded_users = dynspread(shaded_users)

    # ==========================================
    # 4. OVERLAY VECTORS (Towers & GeoJSON)
    # ==========================================
    # Draw the Base Station Center Points
    # ==========================================
    # 4. OVERLAY VECTORS (Towers & GeoJSON)
    # ==========================================
    # Draw the Base Station Center Points
    tower_lons = [bs.lon for bs in base_stations]
    tower_lats = [bs.lat for bs in base_stations]
    tower_labels = [f"Tower {bs.bs_id}" for bs in base_stations]
    
    towers_points = hv.Points(
        {'Lon': tower_lons, 'Lat': tower_lats, 'Label': tower_labels}, 
        kdims=['Lon', 'Lat'], 
        vdims=['Label']
    ).opts(color='white', size=10, marker='^', tools=['hover'])

    # --- NEW: Helper function to translate GeoJSON for Holoviews ---
    def _extract_polygons(geojson_dict):
        parsed_polys = []
        for feature in geojson_dict.get('features', []):
            ...
            if geom_type == 'Polygon':
                # Get the outer boundary ring
                coords = geom.get('coordinates', [[]])[0]
                parsed_polys.append([(c[0], c[1]) for c in coords])
            elif geom_type == 'MultiPolygon':
                for multi in geom.get('coordinates', []):
                    coords = multi[0]
                    parsed_polys.append([(c[0], c[1]) for c in coords])
        return parsed_polys

    try:
        # Pass the cleanly extracted coordinates to Holoviews
        h3_coords = _extract_polygons(hex_geojson)
        bs_coords = _extract_polygons(bs_coverage_geojson)

        h3_grid = hv.Polygons(h3_coords).opts(fill_alpha=0, line_color='gray', line_width=0.5)
        bs_circles = hv.Polygons(bs_coords).opts(fill_alpha=0.1, fill_color='green', line_color='lightgreen', line_width=1)
        
        # Combine the layers
        final_overlay = h3_grid * bs_circles * shaded_users * towers_points
        
    except Exception as e:
        print(f"⚠️ GeoJSON Parsing Error: {e}")
        final_overlay = shaded_users * towers_points

    # ==========================================
    # 5. EXPORT TO HTML
    # ==========================================
    print(f"💾 Compiling interactive UI and saving to {filename}...")
    
    layout = pn.Column(
        pn.pane.Markdown("## Hybrid NTN-TN Coverage Map"),
        final_overlay
    )
    
    layout.save(filename, embed=True)
    print("✅ Master Visualization compiled successfully!")