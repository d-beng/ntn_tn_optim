...
def _process_zone(payload):
    ...
    for cid in range(len(centers)):
        ...
        # Carry the member coords (float32) so the main process can subdivide
        # oversized clusters into multiple towers that each fit the coverage radius.
        results.append((
...
# ----------------------------------------------------------------------
# Cluster subdivision: tile an oversized cluster with multiple towers so
# every tower's fixed coverage radius actually covers its members (no edge
# gaps -> kills "No 5G Tower in Geographic Range" drops).
# ----------------------------------------------------------------------
def _subdivide_to_coverage(pts: np.ndarray, coverage_radius_km: float,
                           seed: int, max_subtowers: int = 64
                           ) -> List[Tuple[float, float, float, int]]:
    """Split member points into enough sub-clusters that each sub-cluster's
    radius <= coverage_radius_km. Returns [(clat, clon, sub_radius_km, size)].

    A single k-means with k = ceil((cluster_radius / coverage_radius)^2) tiles
    the area; we then verify and bump k if any sub-cluster still overflows."""
    if pts is None or len(pts) == 0:
        return []
    center0 = pts.mean(axis=0)
    r0 = _cluster_radius_km_vec(center0, pts)
    if r0 <= coverage_radius_km or len(pts) < 2:
        return [(float(center0[0]), float(center0[1]), float(r0), int(len(pts)))]

    # Area-based initial k: how many coverage-disks tile the cluster disk.
    k = int(math.ceil((r0 / max(coverage_radius_km, 1e-3)) ** 2))
    k = max(2, min(k, max_subtowers, len(pts)))

    out: List[Tuple[float, float, float, int]] = []
    for _ in range(3):  # at most 3 refinement rounds
        labels, centers = _fit_zone_kmeans(pts, k, seed)
        out = []
        overflow = False
        for cid in range(len(centers)):
            mask = labels == cid
            ...
            if len(sub) == 0:
                continue
            c = centers[cid]
            rr = _cluster_radius_km_vec(c, sub)
            out.append((float(c[0]), float(c[1]), float(rr), int(len(sub))))
            if rr > coverage_radius_km * 1.15:   # 15% tolerance
                overflow = True
        if not overflow or k >= min(max_subtowers, len(pts)):
            break
        k = min(int(k * 1.6) + 1, max_subtowers, len(pts))  # grow k, retry
    return out
...
def generate_terrestrial_network(cfg: DictConfig, users: List[User], h3_resolution: int) -> List[BaseStation]:
    ...
    subdivided_extra = 0
    for (clat, clon, raw_radius_km, size, hull, zone_radius_km, zone_size, pts) in raw_candidates:
        ...
        cov_r = float(bs_cfg[scenario_key]["coverage_radius_km"])

        # If the cluster is wider than ONE cell can cover, tile it with several
        # towers so no edge users fall outside coverage. Each sub-tower inherits
        # the same scenario (same local density region).
        if raw_radius_km > cov_r and pts is not None and len(pts) >= 2:
            subtowers = _subdivide_to_coverage(np.asarray(pts), cov_r, random_seed)
            if len(subtowers) > 1:
                subdivided_extra += (len(subtowers) - 1)
            for (sclat, sclon, sr, ssize) in subtowers:
                candidates.append({
                    "lat": sclat, "lon": sclon,
                    "raw_radius_km": sr,
                    "density": real_density,
                    "assigned_user_count": ssize,
                    "membership_boundary": _make_radius_boundary((sclat, sclon), min(sr, cov_r), 24),
                    "zone_radius_km": zone_radius_km,
                    "zone_size": zone_size,
                    "scenario_key": scenario_key,
                    "coverage_radius_km": cov_r,
                })
        else:
            candidates.append({
                "lat": clat, "lon": clon,
    ...
    print(
        ...
        f"{subdivided_extra} extra towers added by subdividing oversized clusters. "
